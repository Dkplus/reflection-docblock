...
final class VarAttributeFormatter implements AttributeFormatter
{
    ...
    {
        $this->formatter = new TypeAttributeFormatter(new RegexAttributeFormatter(
            '/(?P<type>[\S]+)\s*(?P<name>\\$\S+)\s*(?P<description>.*)/',
            '/(?P<type>[\S]+)\s*(?P<description>.*)/'
        ), new TypeResolver());
    }
